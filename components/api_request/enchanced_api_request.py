...
class APIRequestComponent(Component):
    ...
    inputs = [
        TabInput(
            ...
            value="Table",
            info="Select the type of body input: Table (dictionary) or String (raw text/JSON)",
        ...
        TabInput(
            ...
            options=["Table", "String"],
            value="Table",
            info="Select the type of header input: Table (dictionary) or String (raw text)",
            real_time_refresh=True,
        ),
        MessageTextInput(
            name="url_input",
        ...
        MessageTextInput(
            name="body_string",
            display_name="Body",
            info="The body as raw text or JSON.",
        ...
        MessageTextInput(
            name="headers_string",
            display_name="Headers",
            info="The headers as raw text. Example: {\"Authorization\": \"Bearer ...\", \"Content-Type\": \"application/json\"}",
            show=False,  # Will be controlled dynamically
        ),
        IntInput(
            name="timeout",
    ...
    async def make_api_request(self) -> Data:
        ...
        # Body input mode
        body_input_mode = getattr(self, "body_input_mode", "Table")
        if body_input_mode == "Table":
            body = self.body or {}
            body = self._process_body(body)
        else:
            body_string = getattr(self, "body_string", None)
        ...
        # Header input mode
        header_input_mode = getattr(self, "header_input_mode", "Table")
        if header_input_mode == "Table":
            headers = self.headers or {}
        else:
            headers = getattr(self, "headers_string", None)
        headers = self._process_headers(headers, header_input_mode=header_input_mode)
    ...
    def update_build_config(self, build_config: dotdict, field_value: Any, field_name: str | None = None) -> dotdict:
        ...
        if field_name == "body":
            if field_value == "Table":
                build_config["body_input_mode"]["value"] = "Table"
                build_config["body_string"]["show"] = False
            elif field_value == "String":
                build_config["body_input_mode"]["value"] = "String"
                build_config["body_string"]["show"] = True
            return build_config
        # Dynamic control for header input mode if headers is changed
        if field_name == "headers":
            if field_value == "Table":
                build_config["header_input_mode"]["value"] = "Table"
                build_config["headers_string"]["show"] = False
            elif field_value == "String":
                build_config["header_input_mode"]["value"] = "String"
                build_config["headers_string"]["show"] = True
            return build_config
        if field_name != "mode":
