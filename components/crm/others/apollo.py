...
class ApolloOrganizationSearch(Component):
    ...
    - q_organization_domains: str
        Domains separated by comma (ex: "google.com,alphabet.com")

    - q_organization_locations: str
        Locations separated by comma (ex: "San Francisco,New York")

    - q_organization_industries: str
        Industries separated by comma (ex: "Technology,Finance")

    - q_organization_employee_count_ranges: str
        Employee count ranges separated by comma (ex: "1-10,11-50,51-200")

    - q_organization_revenue_ranges: str
        Revenue ranges separated by comma (ex: "0-1M,1M-10M,10M-100M")

    - q_organization_funding_stages: str
        Funding stages separated by comma (ex: "Seed,Series A,Series B")

    - q_organization_technologies: str
        Technologies separated by comma (ex: "Python,React,Node.js")

    - page: int
        Page number (ex: 1)

    - per_page: int
        Results per page (ex: 10)

    - enrich_domain: bool
        Enrich domain information (ex: True or False)
    """
    ...
    inputs = [
        SecretStrInput(name="apollo_api_key", display_name="API Key", required=True, info="Your Apollo API Key."),
