"""
MCP Context Forge Update Gateway Component

This component updates gateway properties in the MCP Context Forge server
by making a PUT request to the /gateways/{gateway_id} endpoint with Bearer token authentication.
"""
...
class UpdateGatewayComponent(Component):
    display_name = "MCP Update Gateway"
    description = "Update gateway properties in the MCP Context Forge server"
    icon = "edit"
    name = "UpdateGatewayComponent"
    ...
    inputs = [
        ...
        StrInput(
            ...
            info="The ID of the gateway to update",
            required=True,
        ),
        StrInput(
            name="gateway_name",
            display_name="Gateway Name",
            info="The new name for the gateway",
            required=False,
        ),
        StrInput(
            name="description",
            display_name="Description",
            info="The new description for the gateway",
            required=False,
        ),
        BoolInput(
            name="enabled",
            display_name="Enabled",
            info="Whether the gateway should be enabled",
            value=True,
            required=False,
        ),
    ]
    ...
    outputs = [
        Output(name="update_result", display_name="Update Result", method="update_gateway"),
    ]
    
    def update_gateway(self) -> Data:
        """
        Update gateway properties in the MCP Context Forge server.
        ...
        try:
            # Execute curl command to update gateway
            curl_command = [
                "curl", 
                "-s",  # Silent mode
                "-X", "PUT",
