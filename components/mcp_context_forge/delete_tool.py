"""
MCP Context Forge Delete Tool Component

This component deletes a tool from the MCP Context Forge server
by making a DELETE request to the /tools/{tool_id} endpoint with Bearer token authentication.
"""
...
class DeleteToolComponent(Component):
    display_name = "MCP Delete Tool"
    description = "Delete a tool from the MCP Context Forge server"
    icon = "trash"
    name = "DeleteToolComponent"
    ...
    inputs = [
        ...
        StrInput(
            ...
            info="The ID of the tool to delete",
            required=True,
        ),
    ]
    
    outputs = [
        Output(name="delete_result", display_name="Delete Result", method="delete_tool"),
    ]
    
    def delete_tool(self) -> Data:
        """
        Delete a tool from the MCP Context Forge server.
        ...
        try:
            # Execute curl command to delete tool
            curl_command = [
                "curl", 
                "-s",  # Silent mode
                "-X", "DELETE",
