...
class SaveToFileComponent(Component):
    ...
    LOCAL_DATA_FORMAT_CHOICES = ["csv", "excel", "json", "markdown", "pdf"]
    LOCAL_MESSAGE_FORMAT_CHOICES = ["txt", "json", "markdown", "pdf"]
    AWS_FORMAT_CHOICES = [
        "txt",
        "json",
        "csv",
    ...
    def _adjust_file_path_with_format(self, path: Path, fmt: str) -> Path:
        ...
        if fmt == "pdf":
            return Path(f"{path}.pdf").expanduser() if file_extension != "pdf" else path
    ...
    def _save_dataframe(self, dataframe: DataFrame, path: Path, fmt: str) -> str:
        ...
        if fmt == "csv":
            ...
            self._save_dataframe_to_pdf(dataframe, path)
        else:
    ...
    def _save_data(self, data: Data, path: Path, fmt: str) -> str:
        ...
        if fmt == "csv":
            ...
            self._save_data_to_pdf(data, path)
        else:
    ...
    async def _save_message(self, message: Message, path: Path, fmt: str) -> str:
        ...
        if fmt == "txt":
            ...
            self._save_text_to_pdf(content, path)
        else:
    ...
    def _save_dataframe_to_pdf(self, dataframe: DataFrame, path: Path) -> None:
        """Save a DataFrame to PDF format using reportlab."""
        try:
            from reportlab.lib import colors
            from reportlab.lib.pagesizes import letter
            from reportlab.platypus import SimpleDocTemplate, Table, TableStyle
        except ImportError as e:
            msg = "reportlab is not installed. Please install it using `uv pip install reportlab`."
            raise ImportError(msg) from e
    ...
    def _save_data_to_pdf(self, data: Data, path: Path) -> None:
        """Save a Data object to PDF format."""
        try:
            from reportlab.lib.pagesizes import letter
        ...
        if hasattr(data, "data") and data.data:
            if isinstance(data.data, dict) or isinstance(data.data, list):
                import json

                content = json.dumps(jsonable_encoder(data.data), indent=2, ensure_ascii=False)
            else:
                content = str(data.data)
        else:
            content = str(data)

        # Try to create a table if data is tabular
        try:
            df = pd.DataFrame(data.data) if hasattr(data, "data") and data.data else pd.DataFrame()
            if not df.empty:
                self._save_dataframe_to_pdf(DataFrame(df), path)
                return
        except Exception:
            pass
    ...
    def _save_text_to_pdf(self, text: str, path: Path) -> None:
        """Save text content to PDF format."""
        try:
            from reportlab.lib.pagesizes import letter
            from reportlab.platypus import Paragraph, SimpleDocTemplate
            from reportlab.lib.styles import getSampleStyleSheet
