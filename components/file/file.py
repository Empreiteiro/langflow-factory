...
class FileComponent(BaseFileComponent):
    ...
    def _safe_log(self, message: str) -> None:
        """Log safely even when component context is unavailable."""
        try:
            self.log(message)
        except Exception:
            logger.warning("FileComponent log unavailable: %s", message)

    def _decode_text_content(self, content: bytes) -> str:
        """Decode bytes to text with a safe fallback chain."""
    ...
    def _is_decode_error_message(self, error: Exception) -> bool:
        message = str(error).lower()
        return "codec can't decode" in message or "charmap" in message
    ...
    def process_files(
        ...
        for file in file_list:
            ...
            if extension in image_extensions:
                ...
                try:
                    ...
                    self._safe_log(f"Could not read file for validation: {e}")
        ...
        def process_file_standard(file_path: str, *, silent_errors: bool = False) -> Data | None:
            try:
                ...
                # Fallback decoding for files with non-UTF encodings (common on Windows)
                self._safe_log(f"Decoding error for {file_path}: {e}. Falling back to safe decoding.")
                try:
                    content = Path(file_path).read_bytes()
                except (OSError, FileNotFoundError) as read_error:
                    self._safe_log(f"Could not read file for fallback decoding: {read_error}")
                    if not silent_errors:
                        raise
                    return None

                text = self._decode_text_content(content)
                return Data(text=text, data={"file_path": file_path})
            except FileNotFoundError as e:
                self._safe_log(f"File not found: {file_path}. Error: {e}")
                if not silent_errors:
                    raise
                return None
            except Exception as e:
                if self._is_decode_error_message(e):
                    self._safe_log(f"Decoding error for {file_path}: {e}. Falling back to safe decoding.")
                    try:
                        content = Path(file_path).read_bytes()
                    except (OSError, FileNotFoundError) as read_error:
                    ...
                    text = self._decode_text_content(content)
                    return Data(text=text, data={"file_path": file_path})
                self._safe_log(f"Unexpected error processing {file_path}: {e}")
                if not silent_errors:
                    raise
                return None
        ...
        if self.advanced_mode and docling_compatible:
            ...
            for file in file_list:
                ...
                if isinstance(doc_rows, list) and doc_rows:
                    ...
                    self._safe_log(f"No text extracted from '{file_path}', creating placeholder data")
                    empty_data = Data(
                        data={
                            "file_path": file_path,
        ...
        self._safe_log(f"Starting parallel processing of {len(file_paths)} files with concurrency: {concurrency}.")
