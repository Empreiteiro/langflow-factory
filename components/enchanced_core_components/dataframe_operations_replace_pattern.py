...
class DataFrameOperationsComponent(Component):
    ...
    inputs = [
        ...
        SortableListInput(
            ...
            options=[
                ...
                {"name": "Replace Pattern", "icon": "regex"},
        ...
        MessageTextInput(
            name="old_pattern",
            display_name="Pattern to Replace",
            info="The pattern/prefix to find and replace (e.g., 'C:\\Users\\Lucas\\Documents\\GitHub\\langflow\\docs\\docs').",
            dynamic=True,
            show=False,
        ),
        MessageTextInput(
            ...
            info="The new pattern/prefix to replace with (e.g., 'https://github.com/langflow-ai/langflow/tree/main/docs/docs').",
            dynamic=True,
            show=False,
        ),
    ]
    ...
    def update_build_config(self, build_config, field_value, field_name=None):
        ...
        if field_name == "operation":
            ...
            elif operation_name == "Replace Pattern":
                build_config["column_name"]["show"] = True
                build_config["old_pattern"]["show"] = True
                build_config["new_pattern"]["show"] = True
    ...
    def perform_operation(self) -> DataFrame:
        ...
        if op == "Replace Pattern":
            return self.replace_pattern(df_copy)
    ...
    def replace_pattern(self, df: DataFrame) -> DataFrame:
        """Replace patterns/prefixes in text while preserving the rest of the text."""
        def replace_pattern_func(text):
            if pd.isna(text):
                return text
            text_str = str(text)
            if text_str.startswith(self.old_pattern):
                # Replace the old pattern with the new pattern, preserving the rest
                remaining_text = text_str[len(self.old_pattern):]
                return self.new_pattern + remaining_text
            return text_str
        
        df[self.column_name] = df[self.column_name].apply(replace_pattern_func)
        return DataFrame(df)
