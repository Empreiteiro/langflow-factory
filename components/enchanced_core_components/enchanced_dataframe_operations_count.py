...
class DataFrameOperationsComponent(Component):
    ...
    inputs = [
        ...
        SortableListInput(
            ...
            options=[
                ...
                {"name": "Filter by Character Count", "icon": "type"},
        ...
        DropdownInput(
            name="char_count_operator",
            display_name="Character Count Operator",
            options=["equals", "greater than", "less than", "greater than or equal", "less than or equal"],
            value="equals",
            info="The operator to apply for filtering by character count.",
            advanced=False,
            dynamic=True,
            show=False,
        ),
        IntInput(
            ...
            info="The number of characters to filter by.",
            dynamic=True,
            show=False,
            value=0,
        ),
    ...
    def update_build_config(self, build_config, field_value, field_name=None):
        ...
        if field_name == "operation":
            ...
            if operation_name == "Filter":
                ...
                build_config["column_name"]["show"] = True
            elif operation_name == "Filter by Character Count":
                build_config["column_name"]["show"] = True
                build_config["char_count_operator"]["show"] = True
                build_config["char_count"]["show"] = True
    ...
    def perform_operation(self) -> DataFrame:
        ...
        if op == "Filter by Character Count":
            return self.filter_by_character_count(df_copy)
        if op == "Sort":
    ...
    def filter_by_character_count(self, df: DataFrame) -> DataFrame:
        column = df[self.column_name].astype(str)
        char_count = getattr(self, "char_count", 0)
        operator = getattr(self, "char_count_operator", "equals")

        # Calculate character count for each row
        char_counts = column.str.len()

        if operator == "equals":
            mask = char_counts == char_count
        elif operator == "greater than":
            mask = char_counts > char_count
        elif operator == "less than":
            mask = char_counts < char_count
            ...
            mask = char_counts == char_count  # Fallback to equals
